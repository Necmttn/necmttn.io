...
exports.createPages = async ({ graphql, actions }) => {
  ...
  const engResult = await graphql(`
    {
      allMdx(filter: { fields: { langKey: { eq: "en" } } }) {
        edges {
          node {
            id
            fileAbsolutePath
  ...
  engPosts.forEach(({ node }, index) => {
    const { slug, layout, langKey } = node.fields;
  ...
  const zhResult = await graphql(`
    {
      allMdx(filter: { fields: { langKey: { eq: "zh" } } }) {
        edges {
          node {
            id
            fileAbsolutePath
  ...
  // Create ZhongWen post pages
  const tilPosts = zhResult.data.allMdx.edges;
  zhPosts.forEach(({ node }, index) => {
    const { slug, layout, langKey } = node.fields;
    const prev = index === 0 ? null : zhPosts[index - 1].node;
    const next = index === zhPosts.length - 1 ? null : zhPosts[index + 1].node;
