...
const Status = styled.span`
  ...
  margin-left: 20px;
  color: #fff;
  background: ${p => (p.color === "active") ? colors.green : colors.blue};
  border-radius: 5px;
  padding: 2px 5px;
  font-size: 1.2rem;
  line-height: 33px;
...
const ChallengeCard: React.FunctionComponent<PostCardProps> = ({ post }) => {
    ...
    <article className={`post-card ${PostCardStyles} ${!post.frontmatter.image ? 'no-image' : ''}`}>
      ...
      <PostCardContent className="post-card-content">
        ...
        <PostCardMeta className="post-card-meta">
          ...
          <Status color={post.frontmatter.meta.status}>status: {post.frontmatter.meta.status} </Status>
          <Streak>Length: {post.frontmatter.meta.length} day</Streak>
        </PostCardMeta>
      </PostCardContent>
    </article>
  );
